...
function i18n.translate(key, data)
	...
	if
		(Spring.GetConfigInt("language_english_unit_names", 1) == 1)
		and key:sub(1, #"units.names.") == "units.names."
	then
		usedLocale = "en"
	end
	...
	for i = 1, #fallbacks do
		...
		if value then
			-- a table-level key (e.g. a group of strings): resolve each sub-key through the
			-- fallback chain too, since a partially translated table is still truthy
