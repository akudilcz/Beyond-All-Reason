-- Per-unit-type movement checks, generated from UnitDefs: every mobile ground unit of
-- the playable factions crosses a flat stretch in parallel lanes. Each unit type must
-- arrive within a generous budget and never exceed its maxSpeed by more than 10%.
local LANES = 24
...
local function isCandidate(def)
	...
	if not md or not md.name or md.name:find("boat") or md.name:find("uboat") or (def.minWaterDepth or 0) > 0 then
		return false -- ships need water; tested separately
	end
	return true
end
