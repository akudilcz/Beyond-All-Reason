...
-- generated from UnitDefs/WeaponDefs, on a flat arena. Each unit gets two independent
...
return {
	...
	run = function(ctx)
		...
		for _, def in pairs(UnitDefs) do
			if isGroundUnit(def) and def.weapons and #def.weapons > 0 and arena.wants(ctx, def.name) then
